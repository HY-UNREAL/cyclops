...
namespace cyclops {
  ...
  using SensorData = std::variant<ImuMockup, ImageData>;
  using TimestampedSensorData = std::tuple<Timestamp, SensorData>;

  static auto makeSensorData(
    std::mt19937& rgen, PoseSignal const& pose_signal,
    SE3Transform const& extrinsic, SensorStatistics const& noise,
    LandmarkPositions const& landmarks) {
    auto imu_timestamps = linspace(0, 10, 1000) | ranges::to_vector;
    auto imu_data = generateImuData(pose_signal, imu_timestamps, rgen, noise);

    auto landmark_timestamps = linspace(0, 10, 100) | ranges::to_vector;
    auto landmark_data = makeLandmarkFrames(
      pose_signal, extrinsic, landmarks, landmark_timestamps);

    std::vector<TimestampedSensorData> result;
  ...
  static auto makeSE3Transform(ImuMotionState const& x) {
    return SE3Transform {
  ...
  static auto projectSO3(Matrix3d const& C) -> Matrix3d {
  ...
  static auto determineSlamOriginFrame(
    ...
    Matrix3d rotation = projectSO3(rotation_sum);
  ...
  private:
    ...
    std::map<Timestamp, ImuMotionState> _estimations;
  ...
  TEST_CASE("Test the whole pipeline") {
    ...
    auto sensor_data =
      makeSensorData(rgen, pose_signal, extrinsic, noise, landmarks);
    ...
    auto x_ws = determineSlamOriginFrame(pose_signal, estimations);
    ...
    THEN("The final state is effectively correct") {
      ...
      auto x_got = compose(x_ws, makeSE3Transform(x_s));
