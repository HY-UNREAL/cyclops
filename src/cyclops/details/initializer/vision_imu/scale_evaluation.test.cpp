#include "cyclops/details/initializer/vision_imu/scale_evaluation.cpp"
...
namespace cyclops::initializer {
  ...
  template <int dim>
  static Vector<dim> randvec(std::mt19937& rgen) {
  ...
  template <int rows, int cols>
  static Matrix<rows, cols> randmat(std::mt19937& rgen) {
    auto rand = std::normal_distribution<double>(0, 1);
  ...
  template <int rows, int cols>
  static Matrix<rows, cols> randchol(std::mt19937& rgen) {
    auto A = randmat<rows, cols>(rgen);
    auto H = (A.transpose() * A).eval();

    Eigen::LLT<Matrix<rows, cols>> _(H);
    return _.matrixU();
  }
