#include "cyclops/details/initializer/vision_imu/solver.imu_only.cpp"
...
namespace cyclops::initializer {
  ...
  class ImuOnlyMatchAcceptDiscriminatorMock:
      public ImuOnlyMatchAcceptDiscriminator {
  public:
    bool determineAccept(ImuMatchCandidate const& match) const override {
      return true;
    }
  ...
  TEST_CASE("Test IMU-only match") {
    ...
    auto solver = ImuOnlyMatchSolverImpl(
      std::move(analysis_mock),
      std::make_unique<ImuOnlyMatchAcceptDiscriminatorMock>(), config);
    ...
    {
      CAPTURE(s_optimized);
      CHECK(std::abs(s_optimized - s) < 1e-3);
    }
    {
      ...
      CHECK(g_optimized.isApprox(g, 1e-3));
    }
    {
      ...
      CHECK(b_a_optimized.isApprox(b_a, 1e-3));
    }
  }
}  // namespace cyclops::initializer
