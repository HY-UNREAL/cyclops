...
namespace cyclops::initializer {
  ...
  static auto makePreintegration(
    ...
    auto result = std::make_unique<ImuPreintegration>(
      Vector3d::Zero(), Vector3d::Zero(), ImuNoise {0.01, 0.01});
  ...
  TEST_CASE("test rotation only visual-inertial matching") {
    ...
    ImuMotions imu_motions;
    imu_motions.emplace_back(ImuMotion {
      .from = 0,
      .to = 1,
      .data = makePreintegration(
    ...
    auto camera_orientations = imu_orientations |
      ...
      ranges::to_vector;
    auto camera_rotation_prior = ImuMatchCameraRotationPrior {
      .rotations =
        std::map<FrameID, Quaterniond> {
    ...
    auto solver = ImuRotationMatchSolverImpl(config);
    auto maybe_rotation_matching = solver.solve(
      imu_motions |
        views::transform([](auto& _) -> ImuMotionRef { return _; }) |
        ranges::to_vector,
